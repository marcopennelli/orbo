...
func main() {
	...
	)
	{
		...
		if addr == "" {
			...
			default:
				fmt.Fprintf(os.Stderr, "invalid host argument: %q (valid hosts: localhost|production)\n", *hostF)
				os.Exit(1)
			}
		}
		timeout = *timeoutF
...
func usage() {
	fmt.Fprintf(os.Stderr, `%s is a command line client for the orbo API.

Usage:
    %s [-host HOST][-url URL][-timeout SECONDS][-verbose|-v] SERVICE ENDPOINT [flags]

    -host HOST:  server host (localhost). valid values: localhost, production
